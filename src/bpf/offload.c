		   ...
		   struct bpf_map *map)
{
	 ...
	 */
	map = compat__bpf_map_inc(map, false);
	if (IS_ERR(map))
		return PTR_ERR(map);
...
static int nfp_bpf_verifier_prep(struct bpf_prog *prog)
{
	...
	nfp_prog->bpf = compat__bpf_prog_get_nn(prog)->app->priv;
#else
...
{
	switch (bpf->command) {
#ifndef COMPAT__BPF_PROG_PREPARE
	case BPF_OFFLOAD_VERIFIER_PREP:
		bpf->verifier.ops = &nfp_bpf_dev_ops;
		return nfp_bpf_verifier_prep(bpf->verifier.prog);
	case BPF_OFFLOAD_TRANSLATE:
		return nfp_bpf_translate(bpf->offload.prog);
	case BPF_OFFLOAD_DESTROY:
		nfp_bpf_destroy(bpf->offload.prog);
		return 0;
#endif
...
#endif
#ifdef COMPAT__BPF_PROG_PREPARE
